...
no_of_shares = 475568395
net_debt_cash = 317600000
discount_rate = 0.0825 # WACC - cost of equity
#=============================================================================================================
def disc_cash_flow(dic_data, input_dic, now=False, fractional=False, dcf_p=dcf_period, disc_rate = discount_rate, dcf_start=dcf_start_year):
    ...
    #============== fractional discount factor, varies with time=========================
        ...
        dcf.loc['Free cash flow', dcf_start] = dcf.loc['Free cash flow', general.recent_end_year+1] * first_factor
...
def fair_value(dic_data, input_dic,now=False, fractional=False,dcf_p=dcf_period,disc_rate = discount_rate, pep_rate = perpetuity_growth_rate, dcf_start=dcf_start_year):
    df = disc_cash_flow(dic_data, input_dic, now=now,fractional=fractional,dcf_p=dcf_p, disc_rate=disc_rate, dcf_start=dcf_start)
    dcf_end = dcf_start + dcf_p# - 1
    if now is True:
        dcf_end = dcf_end -1
