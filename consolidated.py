...
revenue_cols = ['Platform fees', 'Net renewal income', 'Management fees', 'HL Fund AMC', 'Stockbroking income',
                'Interest receivable', 'Adviser charges', 'Funds Library', 'Cash Service', 'Other income',
                'On-going adviser charges']
costs_cols = ['Staff costs', 'Marketing and distribution spend', 'Depreciation, amortisation & financial costs',
              'Office running costs', 'Other costs', 'FSCS levy costs']
...
def aua_analysis(dic_data, input_dic):
    df = combined.total_aua(dic_data, input_dic).reset_index().set_index('month_end').reindex(
    ...
    result.loc[:, 'Total AUA'] = result.loc[:, 'Vantage AUA'] + result.loc[:, 'PMS AUA'] + result.loc[:,
                                                                                           'Cash Service AUA']
...
def nnb_analysis(dic_data, input_dic):
    ...
    df = general.convert_fy_quarter_half_index(df, index=df.index).groupby(
        ['financial_year', 'quarter_no', 'half_no']).sum(min_count=1)
    result = pandas.DataFrame(index=df.index, columns=nnb_cols)
    result.loc[:, 'Vantage nnb'] = df.loc[
                                   :, 'vantage_hl_shares_aua'] + df.loc[
                                   :, 'vantage_other_shares_aua'] + df.loc[
                                   :, 'vantage_other_funds_aua'] + df.loc[
                                   :, 'vantage_hlf_aua'] + df.loc[
                                   :, 'thirdparty_hlf_aua'] + df.loc[
                                   :, 'vantage_cash_aua']

    result.loc[:, 'PMS nnb'] = df['pms_hlf_aua'] + df['pms_others_aua']
    result.loc[:, 'HLMM Funds nnb'] = df.loc[:, 'vantage_hlf_aua'] + df.loc[:, 'thirdparty_hlf_aua'] + df[
        'pms_hlf_aua'] + df['pms_others_aua']
    result.loc[:, 'Cash Service nnb'] = df['cash_service_aua']
    result.loc[:, 'Total nnb'] = result.loc[:, 'Vantage nnb'] + result.loc[:, 'PMS nnb'] + result.loc[:,
                                                                                           'Cash Service nnb']
...
def convert_report_aua_data(year=general.recent_end_year):
    ...
    df.loc[:, 'Vantage AUA'] = final_aua.loc[
                               :, 'vantage_hl_shares_aua'] + final_aua.loc[
                               :, 'vantage_other_shares_aua'] + final_aua.loc[
                               :, 'vantage_other_funds_aua'] + final_aua.loc[
                               :, 'vantage_hlf_aua'] + final_aua.loc[
                               :, 'thirdparty_hlf_aua'] + final_aua.loc[
                               :, 'vantage_cash_aua']

    df.loc[:, 'PMS AUA'] = final_aua.loc[:, 'pms_others_aua'] + final_aua.loc[:, 'pms_hlf_aua']
    df.loc[:, 'HLMM Funds AUA'] = final_aua.loc[:, 'vantage_hlf_aua'] + final_aua.loc[:,
                                                                        'thirdparty_hlf_aua'] + final_aua.loc[:,
                                                                        'pms_others_aua'] + final_aua.loc[:,
                                                                        'pms_hlf_aua']
